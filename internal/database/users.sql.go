...
const loginCheck = `-- name: LoginCheck :one
SELECT id, created_at, updated_at, email, hashed_password, is_chirpy_red FROM users WHERE email = $1
`

func (q *Queries) LoginCheck(ctx context.Context, email string) (User, error) {
	row := q.db.QueryRowContext(ctx, loginCheck, email)
	var i User
	err := row.Scan(
		&i.ID,
		&i.CreatedAt,
